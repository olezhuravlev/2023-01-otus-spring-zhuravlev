...
function saveBook(element) {
    ...
    // Saving new element we don't need to save temporary ID - the real ID will be assigned by database
    // and will replace the temporary ID after that.
    let params = {
        id: isNewElement(host) ? null : bookId,
        title: title,
        authorId: authorId,
        genreId: genreId,
    };
...
function setBookValues(element, json) {
    ...
    if (json.authorId) {
        appendOption(selectAuthorElement, json.authorId, json.authorName);
    } else {
    ...
    selectGenreElement.innerHTML = "";
    if (json.genreId) {
        appendOption(selectGenreElement, json.genreId, json.genreName);
    } else {
...
function saveBookComment(element) {
    ...
    // and will replace the temporary ID after that.
    let params = {
        id: isNewElement(host) ? null : commentId,
        text: commentText,
